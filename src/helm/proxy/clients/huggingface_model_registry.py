...
class HuggingfaceModelQuantizationConfig:
    model_loader: Optional[ModelLoader]
    """Type of the model loader, either AutoModelForCausalLM (HF), AutoAWQForCausalLM (AutoAWQ), or
    AutoGPTQForCausalLM (AutoGPTQ)"""
...
class HuggingFaceHubModelConfig:
    ...
    @staticmethod
    def from_string(raw: str, **kwargs) -> "HuggingFaceHubModelConfig":
...
class HuggingFaceLocalModelConfig:
    ...
    @staticmethod
    def from_path(path: str, **kwargs) -> "HuggingFaceLocalModelConfig":
        ...
        return HuggingFaceLocalModelConfig(model_name=model_name, path=path, **kwargs)
...
def register_huggingface_hub_model_config(model_name: str, **kwargs) -> HuggingFaceHubModelConfig:
    ...
    config = HuggingFaceHubModelConfig.from_string(model_name, **kwargs)
...
def register_huggingface_local_model_config(path: str, **kwargs) -> HuggingFaceLocalModelConfig:
    """Register a AutoModelForCausalLM model from a local directory for later use.

    path: a path to your HF model"""
    config = HuggingFaceLocalModelConfig.from_path(path, **kwargs)
    if config.model_id in _huggingface_model_registry:
